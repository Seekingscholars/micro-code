  ...
  <el-dialog v-dialog-drag :show-close="false" :title="title" append-to-body v-bind="$attrs" @open="onOpen">
    <el-tabs v-model="activeTab" class="no-box-shadow no-padding" type="border-card">
      <el-tab-pane label="请求配置" name="info">
        <el-form ref="form" :model="modelForm" :rules="rules" label-position="right" label-width="80px">
          ...
          <el-form-item label="超时时长" prop="timeout">
            <el-input v-model="modelForm.timeout"></el-input>
          </el-form-item>
      ...
      <el-tab-pane label="请求头信息" name="header">
        ...
        <code-editor v-model="modelForm.header" :mode="'javascript'" :readonly="false"></code-editor>
        <el-alert :closable="false" title="}" type="info"></el-alert>
      </el-tab-pane>
      <el-tab-pane label="请求参数" name="param">
        <el-alert :closable="false" title="{" type="info"></el-alert>
        <code-editor v-model="modelForm.param" :mode="'javascript'" :readonly="false"></code-editor>
        <el-alert :closable="false" title="}" type="info"></el-alert>
      </el-tab-pane>
      <el-tab-pane label="请求体" name="body">
        <el-alert :closable="false" title="{" type="info"></el-alert>
        <code-editor v-model="modelForm.body" :mode="'javascript'" :readonly="false"></code-editor>
        <el-alert :closable="false" title="}" type="info"></el-alert>
      </el-tab-pane>
      <el-tab-pane label="请求后成功处理函数" name="success">
        <el-alert :closable="false" title="function(res){" type="info"></el-alert>
        <code-editor v-model="modelForm.successFunction" :mode="'javascript'" :readonly="false"></code-editor>
        <el-alert :closable="false" title="}" type="info"></el-alert>
      </el-tab-pane>
      <el-tab-pane label="请求后失败处理函数" name="fail">
        <el-alert :closable="false" title="function(error){" type="info"></el-alert>
        <code-editor v-model="modelForm.failFunction" :mode="'javascript'" :readonly="false"></code-editor>
        <el-alert :closable="false" title="}" type="info"></el-alert>
      </el-tab-pane>
    </el-tabs>
    <div slot="footer" class="dialog-footer">
      <el-button :loading="buttonLoading" plain type="primary" @click="submitForm">测 试</el-button>
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.modelForm.id) {
            updateRestApi(this.modelForm).then(res => {
            ...
            addRestApi(this.modelForm).then(res => {
